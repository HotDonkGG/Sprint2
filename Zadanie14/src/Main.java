...
public class Main {
    ...
    private static void changePointAndDistance(Car car, int points, int distance) {
        ...
        if ((distance <= 15) && (userCar.acceleration > opponentCar.acceleration) || (distance > 50) && (userCar.maxSpeed > opponentCar.maxSpeed)) { // если победил на короткой или на длинной дистанции
            System.out.println("Вы выиграли!");
            // Найдите и верните наибольшее из максимальных скоростей
            Double.max(userCar.maxSpeed, opponentCar.maxSpeed);
            return userCar.score+100;
        } else if (((distance <= 15) && (userCar.acceleration == opponentCar.acceleration) || (distance > 50) && (userCar.maxSpeed == opponentCar.maxSpeed))) { // Уровни ускорения должны быть равны
            System.out.println("Ничья!");
            return userCar.score+0;
        } else {
            // Сравните уровни закиси азота
            if (((distance <= 15) && (userCar.nitroLevel > opponentCar.nitroLevel) || (distance > 50) && (userCar.nitroLevel > opponentCar.nitroLevel))) {
